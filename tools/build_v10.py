"""Build v10 copy pass on top of the verified v09 release compiler."""
import hashlib
import json
import runpy
from pathlib import Path

runpy.run_path('tools/build_v09.py', run_name='__main__')
out = Path('dist')
index = out / 'index.html'
html = index.read_text(encoding='utf-8')
assert '09-ci-' in html
html = html.replace('09-ci-', '10-ci-', 1)
index.write_text(html, encoding='utf-8')

manifest = json.loads((out / 'release.json').read_text(encoding='utf-8'))
manifest['release'] = '10-ci-' + manifest['sourceSha'][:12]
files = []
