"""Functional release gate. External application calls are intercepted; no real lead is created."""
...
def install_api_routes(context,lead_requests,event_requests,mode):
    ...
    def leads(route,request):
        if request.method=='OPTIONS':return route.fulfill(status=204,headers=cors)
        lead_requests.append(json.loads(request.post_data or '{}'))
        if mode['fail']:
            return route.fulfill(status=503,headers={**cors,'content-type':'application/json'},body=json.dumps({'error':'Временная ошибка тестового API'}))
        return route.fulfill(status=201,headers={**cors,'content-type':'application/json'},body=json.dumps({'ok':True,'accepted':True,'id':'test-lead','notification':'queued'}))
    def events(route,request):
        if request.method=='OPTIONS':return route.fulfill(status=204,headers=cors)
        event_requests.append(json.loads(request.post_data or '{}'));return route.fulfill(status=204,headers=cors)
    context.route('https://app.samaiconsulting.ru/api/public/landing-leads',leads)
    context.route('https://app.samaiconsulting.ru/api/public/landing-events',events)
...
try:
  with sync_playwright() as p:
    for engine in ['chromium','firefox','webkit']:
      ...
      response=page.goto('http://127.0.0.1:8765/?utm_source=ci&utm_medium=test&utm_campaign=v05',wait_until='load')
      ...
      check(engine+' no external Google font request',not any('fonts.googleapis.com' in u or 'fonts.gstatic.com' in u for u in requests))
      ...
      check(engine+' test page stays noindex','noindex' in (page.locator('meta[name="robots"]').get_attribute('content') or ''))
      ...
      page.locator('#company').fill('CI Test')
      page.locator('#task').fill('Проверка формы без создания реального лида')
      page.locator('#privacy-consent').check()
      page.locator('.form-submit').click()
      page.wait_for_function('document.querySelector("#form-status")?.dataset.state === "success"')
      check(engine+' real form success UI','Заявка отправлена.' in page.locator('#form-status').inner_text())
      ...
      check(engine+' lead idempotency key',len(payload.get('idempotencyKey',''))>=20)
      check(engine+' consent not copied into payload','privacy_consent' not in payload)
      check(engine+' analytics excludes PII',all('release-check@example.invalid' not in json.dumps(x) and 'Тест сборки' not in json.dumps(x,ensure_ascii=False) for x in events))
      check(engine+' conversion events emitted',any(x.get('event')=='form_submit_success' for x in events))

      mode['fail']=True
      page.locator('#name').fill('Тест ошибки')
      page.locator('input[name="method"][value="email"]').check()
      page.locator('#contact-value').fill('retry-check@example.invalid')
      ...
      failed_key=leads[-1]['idempotencyKey'];page.locator('.form-submit').click();page.wait_for_timeout(250)
      check(engine+' retry keeps idempotency key',leads[-1]['idempotencyKey']==failed_key)
      ...
      legal=context.new_page();check(engine+' privacy page',(legal.goto('http://127.0.0.1:8765/privacy.html')).status==200);check(engine+' privacy operator','Самай Сергей Леонидович' in legal.locator('body').inner_text());check(engine+' consent page',(legal.goto('http://127.0.0.1:8765/consent.html')).status==200);legal.close()
      context.tracing.stop(path=str(out/(engine+'-trace.zip')));context.close()

      reduced=browser.new_context(reduced_motion='reduce',viewport={'width':390,'height':844});page=reduced.new_page();last_page=page;page.on('pageerror',lambda e:errors.append(str(e)));page.goto('http://127.0.0.1:8765/',wait_until='load');page.wait_for_function('window.SamaiMotion');check(engine+' OS reduced motion',page.evaluate('window.SamaiMotion.mode')=='off');check(engine+' reduced motion content readable',page.locator('h1').is_visible());reduced.close()
      plain=browser.new_context(java_script_enabled=False);page=plain.new_page();last_page=page;page.goto('http://127.0.0.1:8765/',wait_until='load');check(engine+' no-JS content visible',page.locator('h1').is_visible());check(engine+' no-JS form remains disabled',page.locator('.form-submit').is_disabled());check(engine+' no-JS legal link',page.locator('a[href="privacy.html"]').count()>=1);plain.close();browser.close();last_page=None
