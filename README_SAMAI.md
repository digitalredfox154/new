...
Pushes run: JS syntax -> immutable build -> Chromium/Firefox/WebKit browser gates -> tested release branch `samai-landing-releases`. Artifacts contain checks, screenshots and browser traces. Failed checks block publication.
...
Local checks: Python 3.12+, Node 22, `pip install -r requirements-ci.txt`, `python -m playwright install --with-deps chromium firefox webkit`, `python tools/build_v09.py`, `python tools/browser_gate.py`, `python tools/v08_gate.py`, `python tools/v09_gate.py`.
